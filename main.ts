...
namespace Microbit {
    ...
    export function hState(hallpin: Read_pin): number {
        ...
        return pins.analogReadPin(pin18);
    }
...
/*
    ...
    //% block="摇杆检测上下%sxpin左右%zypin方向为%gs"
    //% weight=27
    //% sxpin.fieldEditor="gridpicker"
    //% sxpin.fieldOptions.width=220
    //% sxpin.fieldOptions.columns=1
    //% zypin.fieldEditor="gridpicker"
    //% zypin.fieldOptions.width=220
    //% zypin.fieldOptions.columns=1
    //% subcategory=传感器 
    export function rocker(sxpin:Read_pin,zypin:Read_pin, gs: gs): boolean {
        let pin58
        let zy
        let sx
        let z
        ...
        z = pins.analogReadPin(zy);
        s = pins.analogReadPin(sx);
    ...
    //% blockId=powerbrick_rfidprobe block="RFID 开始检测"
    //% weight=26
    //% group="RFID" 
    //% subcategory=传感器
    ...
    //% blockId=powerbrick_onrfidpresent block="RFID 检测到卡片并等待"
    //% weight=26
    //% group="RFID" 
    //% subcategory=传感器
    ...
    //% blockId=powerbrick_rfiduuid block="RFID 读取卡号"
    //% weight=26
    //% group="RFID"
    //% subcategory=传感器
    ...
    //% blockId=powerbrick_rfidwrite block="RFID 向区|%sector 块|%block 写入文本|%txt"
    //% weight=26
    //% group="RFID" 
    //% subcategory=传感器
    ...
    //% blockId=powerbrick_rfidread block="RFID 读区|%sector 块|%block的内容"
    //% weight=26
    //% group="RFID" 
    //% subcategory=传感器
...
**/
    ...
    export function Servo4(num: enServo, value: number): void {
        ...
        // 50hz: 20,000 us
        let us = ((value - 180) * 20 / 3 + 1500); // 0.6 ~ 2.4
        ...
        if (pins.i2cReadNumber(_I2CAddr, NumberFormat.Int8LE) == 67) {
            ...
            for (let r = 0; r < s.length; r++) {
                ...
                for (let l = 0; l < 5; l++) {
                    ...
                    ind2 = (oled_x + r) * 5 * (_ZOOM + 1) + oled_y * 128 + l * (_ZOOM + 1) + 1
                    if (color == 0)
                        col2 = 255 - col2
                    _screen[ind2] = col2
                    if (_ZOOM)
...
/*
    ...
    //% blockId="sound recording" block="录音模块 %rpin "
    //% magState.shadow="toggleOnOff"
    //% weight=1
    //% rpin.fieldEditor="gridpicker"
    //% rpin.fieldOptions.width=220
    //% rpin.fieldOptions.columns=2
    //% expandableArgumentMode="toggle"
    //% subcategory=执行器
    export function Record(rpin: Recodpin): void {

        let readpin
        let writepin
        if (rpin == 13) {
            readpin = AnalogPin.P0;
            writepin = DigitalPin.P13;
        }
        if (rpin == 114) {
            readpin = AnalogPin.P1;
            writepin = DigitalPin.P14;
        }
        if (rpin == 215) {
            readpin = AnalogPin.P2;
            writepin = DigitalPin.P15;
        }
